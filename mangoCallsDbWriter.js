...
const getCallsInRange = (dateFrom, dateTo) => {
    ...
    console.log(`Получаем данные за период c ${moment(dateFrom).format('LLL')} по ${moment(dateTo).format('LLL')}`.red);
...
const getCallsPerMonth = (date) => {
    console.log(`Получаем данные за месяц на основе этой даты ${moment(date).format('ll')}`.red);
...
const getCallsPerDay = (date) => {
    console.log(`Получаем данные за день на основе этой даты ${moment(date).format('ll')}`.red);
